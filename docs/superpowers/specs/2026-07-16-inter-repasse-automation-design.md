...
- Terminal: `pago`. Books nothing new in the ledger — the existing `aprovarRepasse` ledger effects remain as-is; `pago` is operational confirmation. (If the current implementation books the `resgatado` lancamento at `aprovado`, that stays; a `falhou` after `aprovado` must compensate — implementation must verify where the debit is booked and, if at approval time, add a compensating credit on `falhou`. Flagged as an implementation checkpoint, not a design change.)
