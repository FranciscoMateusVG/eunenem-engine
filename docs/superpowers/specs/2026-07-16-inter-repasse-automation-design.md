...
4. Worker (`repasse.confirmar`):
   - With `codigoSolicitacao`: poll `consultarPagamento` → terminal `pago`/`rejeitado`/`cancelado` resolves the FSM accordingly (`rejeitado`/`cancelado` → `falhou`, retryable by admin). Safe by construction — no matching involved.
   - Without it (crash between Inter's response and our codigo-persist — rare by construction, the intent row commits pre-call): **AMENDED 2026-07-16 (Rex's Inter-API finding, `ju5w2`): Inter has NO caller-supplied field that reliably round-trips** (only `descricao` ≤140 chars goes out, unconfirmed whether it returns as `descricaoPix`; no idempotency key exists). Therefore `buscarPagamentos` **never auto-books `pago`**: referencia is embedded in `descricao` best-effort; **zero candidates sustained across the full 48h escalation → auto-`falhou`** (extrato absence over 48h is real evidence); **≥1 candidate at any point → stays `verificando` flagged needs-manual-resolution** with the candidate list persisted. Resolution is human: admin mutations `resolverManualPago` (requires the matched codigoSolicitacao; books identically to auto-pago) and `resolverManualFalhou` (positive no-payment assertion), both audit-logged, both legal only from the flagged state. Upgrade path: a sandbox/prod round-trip confirming `descricao`→`descricaoPix` survival permits future auto-resolve on descricao match — until confirmed, manual.
