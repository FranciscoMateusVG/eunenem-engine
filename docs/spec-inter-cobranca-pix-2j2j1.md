...
**D2 (2026-08-05, found by Rex during B4-adjacent verification; adjudicated by GLaDOS — supersedes the §5 "daily `GET /webhook/callbacks` audit sweep" line):**
The §5 reconciliation-poller section prescribed a daily `GET /webhook/callbacks` audit sweep. That endpoint does NOT exist on Inter's Pix API v2 — it belongs to a different Inter API family (Banking v2 / Cobrança Boleto+Pix v3) with a different base API, scopes, and credential set. The spec inherited it from the research fact sheet without a per-endpoint API-family check.

**Resolution (adjudicated):** the daily callback-audit sweep is DROPPED from B4's acceptance. The 5-minute reconciliation poll (`GET /cob/{txid}` for open/expired charges) carries the correctness guarantee on its own — it was always the primary mechanism; the audit sweep was observability, not correctness. If daily callback-delivery observability is ever wanted, it is a separate future integration against the correct API family, not part of this build. B4 and B8 implementers: do not build against the superseded line.

**D3 (2026-08-06, raised by Cipher; adjudicated by GLaDOS — refines §7's "byte-level assertion on the archived webhook rows"):**
The §7 E2E requirement prescribed byte-level assertions on archived webhook rows. The archive column is JSONB, which structurally cannot preserve the original wire bytes (whitespace, key order are normalized on write) — a literal byte assertion is unimplementable against that storage.

**Resolution (adjudicated):** archive assertions target the **canonical bounded JSON projection serialization** — i.e., assert equality of the canonicalized, size-bounded JSON projection of the payload, not raw wire whitespace/key order. Canonical equality preserves the audit/security value the byte-level language was reaching for (content integrity, no silent field loss); raw-byte fidelity was never achievable through JSONB and is not a goal. B8 implementers assert against the canonical projection.
